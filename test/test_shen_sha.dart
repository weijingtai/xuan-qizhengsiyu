...
void main() {
  group("太岁驾前轮宫煞", () {
    test("子年", () {
      ...
      expect(
        resultMapper[EnumTwelveGong.Zi]!.any((t) => t.order != 0),
        equals(false),
      );
      expect(
        resultMapper[EnumTwelveGong.Chou]!.any((t) => t.order != 1),
        equals(false),
      );
      expect(
        resultMapper[EnumTwelveGong.Hai]!.any((t) => t.order != 11),
        equals(false),
      );
    });
    test("亥年", () {
      ...
      expect(
        resultMapper[EnumTwelveGong.Zi]!.any((t) => t.order != 1),
        equals(false),
      );
      expect(
        resultMapper[EnumTwelveGong.Chou]!.any((t) => t.order != 2),
        equals(false),
      );
      expect(
        resultMapper[EnumTwelveGong.Hai]!.any((t) => t.order != 0),
        equals(false),
      );
      expect(
        resultMapper[EnumTwelveGong.Xu]!.any((t) => t.order != 11),
        equals(false),
      );
    });
    test("丑年", () {
      ...
      // print(resultMapper[EnumTwelveGong.Zi]!.first.order);
      expect(
        resultMapper[EnumTwelveGong.Zi]!.any((t) => t.order != 11),
        equals(false),
      );
      expect(
        resultMapper[EnumTwelveGong.Chou]!.any((t) => t.order != 0),
        equals(false),
      );
      expect(
        resultMapper[EnumTwelveGong.Hai]!.any((t) => t.order != 10),
        equals(false),
      );
      expect(
        resultMapper[EnumTwelveGong.Xu]!.any((t) => t.order != 9),
        equals(false),
      );
    });
  });
  ...
  group("太岁驾后轮宫煞", () {
    ...
    test("丑年", () {
      ...
      expect(
        resultMapper[EnumTwelveGong.Yin]!.first,
        EnumAfterTaiSuiShenSha.HongLuan,
      ...
      expect(
        resultMapper[EnumTwelveGong.Shen]!.first,
        EnumAfterTaiSuiShenSha.TianXi,
        reason: resultMapper[EnumTwelveGong.Shen]!.first.toString(),
      );
      expect(
        resultMapper[EnumTwelveGong.You]!.length,
        equals(3),
  ...
  group("马前诸煞", () {
    test("亥", () {
      ...
      expect(resultMapper.length, equals(12));
      expect(
        resultMapper[EnumTwelveGong.Hai]!,
        EnumShenShaBeforeHouseStar.YiMa,
      );
      expect(resultMapper[EnumTwelveGong.Zi]!, EnumShenShaBeforeHouseStar.LiuE);
      expect(
        resultMapper[EnumTwelveGong.Chou]!,
  ...
  final monthDiZhiJsonFile = File(
    '$projectRoot/assets/shen_sha/74_shensha_dizhi_month.json',
  );
  ...
  List<MonthDiZhiShenSha> monthDiZhiShenSha = monthDiZhiList
      .map((e) => MonthDiZhiShenSha.fromJson(e))
      .toList();

  final ganzhiJsonFile = File(
    '$projectRoot/assets/shen_sha/74_shensha_ganzhi.json',
  );
  final ganzhiJsonString = ganzhiJsonFile.readAsStringSync();
  final ganzhiList = json.decode(ganzhiJsonString) as List;
  List<GanZhiShenSha> ganzhiShenSha = ganzhiList
      .map((e) => GanZhiShenSha.fromJson(e))
      .toList();

  final bundledShenShaJsonFile = File(
    '$projectRoot/assets/shen_sha/74_shensha_bundle.json',
  );
  ...
  List<BundledShenSha> bundledShenSha = bundledShenShaList
      .map((e) => BundledShenSha.fromJson(e))
      .toList();

  final otherShenShaJsonFile = File(
    '$projectRoot/assets/shen_sha/74_shensha_others.json',
  );
  ...
  List<OtherShenSha> otherShenSha = otherShenShaList
  ...
  group("天干神煞", () {
    ...
    test("丁丑年", () async {
      ...
      final ganZhiMapper = await shenShaManager.generateGanZhiShenShaMapper(
        JiaZi.DING_CHOU,
      );
  ...
  group("斗杓", () {
    test("斗杓", () {
      EnumTwelveGong gong = ShenShaManager.generateDouBiao(
        JiaZi.JI_MAO,
        JiaZi.WU_WU,
      );
  ...
  group("驾后", () {
    ...
    for (int i = 0; i < 4; i++) {
      test("驿马在${yiMaList[i].name}", () async {
        ...
        final List<DiZhi> diZhiList = CollectUtils.changeSeq(
          diZhi,
          DiZhi.values,
        );
        for (int j = 0; j < 12; j++) {
